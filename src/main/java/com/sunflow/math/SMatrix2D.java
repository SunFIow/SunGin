 ...
 * Matrices are used to describe a transformation; see {@link SMatrix_D} for a
 ...
 */
public class SMatrix2D implements SMatrix_D {
	...
	public SMatrix2D(float m00, float m01, float m02,
			float m10, float m11, float m12) {
		set(m00, m01, m02,
				m10, m11, m12);
	}

	public SMatrix2D(SMatrix_D matrix) {
		set(matrix);
	}
	 ...
	 */
	@Override
	public SMatrix2D get() {
		SMatrix2D outgoing = new SMatrix2D();
		outgoing.set(this);
		return outgoing;
	}
	 ...
	 *             If <tt>matrix</tt> is not 2D.
	 */
	@Override
	public void set(SMatrix_D matrix) {
		if (matrix instanceof SMatrix2D) {
			SMatrix2D src = (SMatrix2D) matrix;
	...
	@Override
	public void apply(SMatrix_D source) {
		if (source instanceof SMatrix2D) {
			apply((SMatrix2D) source);
		} else if (source instanceof SMatrix3D) {
			apply((SMatrix3D) source);
		}
	}

	@Override
	public void apply(SMatrix2D source) {
	 ...
	 */
	@Override
	public void apply(SMatrix3D source) {
		throw new IllegalArgumentException("Cannot use apply(SMatrix3D) on a PMatrix2D.");
	}
	 ...
	 */
	@Override
	public void preApply(SMatrix_D source) {
		if (source instanceof SMatrix2D) {
			preApply((SMatrix2D) source);
		} else if (source instanceof SMatrix3D) {
			preApply((SMatrix3D) source);
		}
	}

	@Override
	public void preApply(SMatrix2D left) {
	 ...
	 */
	@Override
	public void preApply(SMatrix3D left) {
		throw new IllegalArgumentException("Cannot use preApply(SMatrix3D) on a PMatrix2D.");
	}
	 ...
	 */
	@Override
	public SVector mult(SVector source, SVector target) {
		if (target == null) {
			target = new SVector();
		}
	...
	//////////////////////////////////////////////////////////////
	static private final float max(float a, float b, float c) {
