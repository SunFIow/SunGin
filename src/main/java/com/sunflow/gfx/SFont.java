...
public class SFont implements SConstants {
	 ...
	 */
	public SFont(Font font, boolean smooth) {
	 ...
	 * @nowebref
	 * @param charset
	 *            array of all unicode chars that should be included
	 */
	public SFont(Font font, boolean smooth, char charset[]) {
...
//      lazyFont = font;
			...
			if (glyphCount != charset.length) {
				glyphs = (Glyph[]) GameBase.subset(glyphs, 0, glyphCount);
			}
...
//      }
	 ...
	 *            InputStream
	 */
	public SFont(InputStream input) throws IOException {
...
//    findNative();
	...
	protected void addGlyph(char c) {
		...
		if (glyphCount == glyphs.length) {
			glyphs = (Glyph[]) GameBase.expand(glyphs);
		}
		if (glyphCount == 0) {
			glyph.index = 0;
...
//  }
	...
	public SShape getShape(char ch, float detail) {
		...
		PathIterator iter = (detail == 0) ? shp.getPathIterator(null) : // maintain curves
...
//      PApplet.println(iterPoints);
	...
	static public Font findFont(String name) {
		...
		if (!name.equals(systemFontName) &&
			...
			SGraphics.showWarning("\"" + name + "\" is not available, " +
	...
	public class Glyph {
		...
		protected void readBitmap(DataInputStream is) throws IOException {
			image = new SImage(width, height, ALPHA);
